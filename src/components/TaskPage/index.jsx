...
export default function TaskPage({ taskId = null, initialTask = null }) {
  ...
  useEffect(() => {
    ...
    async function fetchTask() {
      ...
      try {
        // НЕ вызываем Storage.init(); только берём токен, если он уже сохранён
        const token = await Storage.getToken()
        const profile = await Storage.getProfile()
        const myId = profile.id
        console.warn("НОВАЯ ВЕРСИЯ")
        if (!token) {
          // Токен не найден — не продолжаем. Можно изменить: попытаться без токена, если API допускает.
          throw new Error('Токен авторизации не найден. Пожалуйста, войдите в аккаунт.')
        }

        const res = await getTask(taskId, token)

        if (mounted) setTask(res)
      } catch (err) {
          ...
          <>
            <div className="sheet-task__user-info user-info-accepted">
              <div className="user-info-personal">
                {/* Если у тебя есть данные о пользователе — подставь сюда реальный avatar/name */}
                <img
                  className="user-info__avatar"
                  src={task?.needyAvatar || "https://sun9-67.userapi.com/s/v1/ig2/CY_xDesKnMtl0OiJynK0oc7QnxQVJUgeciJSi_MpZUiE3EHSCNltr76jugXaygGd2Xh0M8-61v7Jwfl1kO87YWVe.jpg"}
                  alt="Аватар"
                />
                <h5 className="h5 inter-500 user-info__text">{task?.needyName || (needy ? `Пользователь #${needy}` : 'Автор')}</h5>
              </div>

              <TaskCategory className="user-info__status" type={categoryType} />

              {/* если есть ссылка на профиль — подставляй; пока оставляем как заглушка */}
              <a className="user-info-accepted__text h5 inter-700 color-white" href={task?.needyProfileUrl || '#'} target="_blank" rel="noreferrer">Написать</a>
            </div>
