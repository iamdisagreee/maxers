...
export default function TaskCard({ type }) {
  return (
    <div>
        ...
        <div className={`task-card ${type}`}>
            <div className="task-card-top">
                <TaskStatus type='cancelled'></TaskStatus>
            </div>
            <div className="task-card-center">
                <div className="task-card__title inter-700 h4">Купить лекарства в аптеке</div>
                <div className="task-card__location">
                    <img src={personIcon} alt="Местоположение" className='card-location__icon' />
                    <span className='inter-500 h6'>волонтёр: Алексей Б.</span>
                </div>
            </div>
            <div className="task-card-bottom">
                <div className="task-card__date inter-400 h6">создано 10 минут назад</div>
            </div>
            <div className='task-card-category-big'>
                <PillIcon className="task-category__big-icon" style={{ fill: 'var(--dprimary)' }}></PillIcon>
                {/* <img
                src={CATEGORY_MAP["pill"].icon}
                alt={CATEGORY_MAP["pill"].text}
                className="task-category__big-icon"
                /> */}
            </div>
        </div>
        )}
